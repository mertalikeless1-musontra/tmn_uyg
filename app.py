...
def create_turkish_synthetic_data(N=10000):
    ...
    # Türkiye İşgücü Piyayasını Temsilen Kategorik Özellikler
    ...
    def calculate_placement_prob(row):
        ...
        # A) BÖLÜM VE NOT ORTALAMASI ETKİSİ
        if row['Bolum'] in ['Tıp', 'Yazılım/Bilgisayar Müh.']:
            prob += 0.35
        elif row['Bolum'] in ['İşletme/İktisat', 'Hukuk']:
            prob += 0.15
        else:
            prob -= 0.10
        ...
        # B) ÜNİVERSİTE TİPİ ETKİSİ
        if row['Uni_Tipi'] == 'Devlet (Top 5)':
            prob += 0.15
        elif row['Uni_Tipi'] == 'Vakıf (Yüksek)':
            prob += 0.10
            
        # C) DENEYİM VE EK YETKİNLİKLER
...
# -------------------------- 2. HİPERPARAMETRE OPTİMİZASYONU (RANDOMIZED SEARCH) --------------------------
...
# -------------------------- 4. MODELİ KAYDETME --------------------------
# Bu, app.py'nin okuyacağı model dosyalarını oluşturur.
joblib.dump(best_lgbm_model, 'lgbm_model.pkl')
joblib.dump(scaler, 'scaler.pkl')
joblib.dump(X.columns.tolist(), 'feature_names.pkl') 

print("\n✅ Yeni Model, Ölçekleyici ve ÖZELLİK LİSTESİ başarıyla yerel diske KAYDEDİLDİ.")
